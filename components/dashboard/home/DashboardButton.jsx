...
function ButtonMakeEvent({role, onPress}) {
    return <TouchableOpacity onPress={onPress}
                             className={role === "ROLE_CUSTOMER" ? "w-full p-5 h-[190px] bg-[#00F279] rounded-xl flex flex-col justify-between" : "w-1/2 p-5 h-[190px] bg-[#78F3B5] rounded-xl flex flex-col justify-between"}>
...
function ButtonListRequests({onPress, role}) {
    return <TouchableOpacity onPress={onPress}
                             className={role === "ROLE_CUSTOMER" ? "hidden" : "w-1/2 p-5 h-[190px] bg-[#00F279] rounded-xl flex flex-col justify-between"}>
        <View className="p-4 bg-white rounded-full max-w-[60px]">
            <MaterialCommunityIcons name="cart" size={30}/>
        </View>
